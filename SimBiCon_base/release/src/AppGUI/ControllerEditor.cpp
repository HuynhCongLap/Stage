...
void ControllerEditor::drawDesiredTarget(){
	...
	float tempColor[] = {0.5, 1.0, 0.5, 1.0};
...
void ControllerEditor::draw(bool shadowMode){
	...
	if (shadowMode == false){
		flags |= SHOW_COLOURS; // flags |= SHOW_COLOURS
		...
		if (Globals::drawContactForces){
			...
			float tempColor[] = { 0.5, 0.0, 0.5, 1.0 };
			glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, tempColor);
			SimBiController* con = conF->getController();
			DynamicArray<ContactPoint>* cfs = conF->getWorld()->getContactForces();
			for (uint i=0;i<cfs->size();i++){
				ContactPoint *c = &((*cfs)[i]);
				
				//GLUtils::drawCylinder(0.01, c->f * 0.0008, c->cp);
				//GLUtils::drawCone(0.03, c->f * 0.0008, c->cp+c->f*0.0008); // draw forces of ODE
				// Volumetric implementation
				tprintf("Geo pos global: %lf %lf %lf\n", c->geoPos.getX(), c->geoPos.getY(), c->geoPos.getZ()); //
				tprintf("Depth: %f \n",  c->d); // depth of penetration

				float cs = -3 * (c->d - 2.0) * (c->d - 2.0) / (4 * c->d - 12);
				Vector3d v_cs = Vector3d(0, 1, 0) * -cs;
				// convert the position to the local position
				Vector3d Vc = c->rb1->getAbsoluteVelocityForLocalPoint(c->geoPos) + c->rb1->getAngularVelocity().crossProductWith(v_cs);
				Vector3d Vcn = Vector3d(0, 1, 0) *(Vc.dotProductWith(Vector3d(0, 1, 0)));
				tprintf("Force Cn: %lf %lf %lf \n", Vcn.getX(), Vcn.getY(), Vcn.getZ());

				float volume = (-1.0 / 3.0) * PI * c->d * c->d * (c->d - 3);

				Vector3d force = Vector3d(0, 1, 0) * volume * (1 + Vcn.length()); //  Normal force
				////////////////////////////////////////////////////////////////////
				tprintf("Force: %lf %lf %lf  -  Volume: %f - Depth: %f \n", force.getX(), force.getY(), force.getZ(), volume, c->d);
				GLUtils::drawCylinder(0.008, force*10000 , c->cp);

			
			}

			Vector3d forceOnLeftFoot = con->getForceOnFoot(con->getRBBySymbolicName("lFoot"), cfs); // force on left foot
			Vector3d forceOnRightFoot = con->getForceOnFoot(con->getRBBySymbolicName("rFoot"), cfs); // force on right foot

			float tempColor1[] = { 1.0, 0.0, 0.0, 1.0 };
			glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, tempColor1);

			footPos.push_back(forceOnLeftFoot);
			footPoint.push_back(con->getRBBySymbolicName("lFoot")->getCMPosition());
			for(int i=0 ; i<footPos.size(); i++)
				//GLUtils::drawCylinder(0.003, footPos[i] * 0.0008, footPoint[i]);
			
		
			//GLUtils::drawCylinder(0.008, forceOnRightFoot * 0.0008, con->getRBBySymbolicName("rFoot")->getCMPosition());

			Vector3d sumFoot = forceOnLeftFoot + forceOnRightFoot;
			float phi = con->getPhase();
			float ratio = con->getStanceFootWeightRatio(cfs);
			//tprintf("Get Force on left foot: %lf %lf %lf\n", forceOnLeftFoot.getX(), forceOnLeftFoot.getY(), forceOnLeftFoot.getZ() );
			//tprintf("Get Force on right foot: %lf %lf %lf\n", forceOnRightFoot.getX(), forceOnRightFoot.getY(), forceOnRightFoot.getZ());
			//tprintf("Sum thiet su: %lf %lf %lf\n", sumFoot.getX(), sumFoot.getY(), sumFoot.getZ());
			//tprintf("Phi: %lf\n", phi);
			//tprintf("Ratio: %lf\n", ratio);

			// this part is to get the data and write to a text file
			if (myfile.is_open())
			{
				//myfile<< count << "\t" << forceOnLeftFoot.getX()  << "\t"<< forceOnLeftFoot.getY() << "\t" << forceOnLeftFoot.getZ() << "\t" << forceOnRightFoot.getX() << "\t" << forceOnRightFoot.getY() << "\t" << forceOnRightFoot.getZ()<< "\t" << sumFoot.getX() << "\t" << sumFoot.getY() << "\t" << sumFoot.getZ() << "\t" << ratio << "\n";
			}
			count++;
		}
	}
