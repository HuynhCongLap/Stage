...
void ODEWorld::processCollisions(dGeomID o1, dGeomID o2){
	...
	for (int i=0;i<num_contacts;i++){
		...
		if (jointFeedbackCount >= MAX_CONTACT_FEEDBACK)
			...
			contactPoints[jointFeedbackCount].d = cps[i].geom.depth;
			

			const dReal* PosGeo = dGeomGetPosition(cps[i].geom.g1);
		    dReal PosGeoGlobal[4];
			dBodyGetRelPointPos(b1, PosGeo[0], PosGeo[1], PosGeo[2], PosGeoGlobal);
			contactPoints[jointFeedbackCount].geoPos = Point3d(PosGeoGlobal[0], PosGeoGlobal[1], PosGeoGlobal[2]);
			dJointSetFeedback(c,&(jointFeedback[jointFeedbackCount]));

			//const dReal* PosGeo = dGeomGetPosition(cps[i].geom.g1);
			//float radius = dGeomSphereGetRadius(cps[i].geom.g1);
			//float depth = cps[i].geom.depth;

		    //dReal ContactPos[4];
			
			//dBodyGetPosRelPoint(b1, cps[i].geom.pos[0], cps[i].geom.pos[1], cps[i].geom.pos[2], ContactPos);
			//dBodyGetRelPointPos
			//Vector3d subtract = Vector3d(ContactPos[0] - PosGeo[0], ContactPos[1] - PosGeo[1], ContactPos[2] - PosGeo[2]);
			//Vector3d ns =  -subtract.unit();
			//tprintf("Depth: %f  - Radius: %f  - Length: %f\n",depth, radius,  subtract.length());
			//tprintf("Vi tri cuc sphere: %f %f %f\n", PosGeo[0], PosGeo[1], PosGeo[2]);

			//float Vs = -1/3.0*PI*depth*depth*(depth-3); // volume of penetration
